 ...
 */
public class AGTGenPerformanceTestAction implements IObjectActionDelegate {
	
	private static final String ACTION_ID = "lazy.test.plugin.popmenu.extend.action.method.performance";
	...
	public void run(IAction action) {
        ...
        try {
            ...
            AGTPerformanceShowVO detailShowVO = WizardVOConvertProcessor.parseAGTPerformanceShowVO(context, selection);
            context.setVo(detailShowVO);

            WizardDialog dialog = WizardDialogGenProcessor.genPerformanceWizardDialog(context, workbench);
            
            if (Boolean.valueOf(MessageConfigHandler.findMappingResult(AGTCommonProperties.PROJECT_CONFIG_WHITEBOX))) {
            	dialog.setMinimumPageSize(100, 420);
            } else {
            	dialog.setMinimumPageSize(100, 450);
            }
